...
Nginx 通过brew安装可以修改默认的80端口为8080，可以不通过sudo启动服务
...
也可以在安装目录中启动,但是需要以管理者方式启动，还是启动默认的80端口

```
安装的目录： /usr/local/Cellar/nginx
...
```
修改默认配置:/usr/local/etc/nginx/nginx.conf
...
### 参考
[nginx配置](https://github.com/jaywcjlove/handbook/blob/master/Centos/nginx%E9%85%8D%E7%BD%AE.md)
